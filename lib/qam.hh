...
struct QuadratureAmplitudeModulation<64, TYPE, CODE> : public Modulation<TYPE, CODE> {
    ...
    void hard(code_type* b, complex_type c)
    {
        ...
        b[4] =
            std::abs(std::abs(c.real()) - amp(4)) < amp(2) ? code_type(-1) : code_type(1);
        b[5] =
            std::abs(std::abs(c.imag()) - amp(4)) < amp(2) ? code_type(-1) : code_type(1);
    }
...
struct QuadratureAmplitudeModulation<256, TYPE, CODE> : public Modulation<TYPE, CODE> {
    ...
    void hard(code_type* b, complex_type c)
    {
        ...
        b[4] =
            std::abs(std::abs(c.real()) - amp(8)) < amp(4) ? code_type(-1) : code_type(1);
        b[5] =
            std::abs(std::abs(c.imag()) - amp(8)) < amp(4) ? code_type(-1) : code_type(1);
    ...
    void soft(code_type* b, complex_type c, value_type precision)
    {
        ...
        b[6] = quantize(
            precision, std::abs(std::abs(std::abs(c.real()) - amp(8)) - amp(4)) - amp(2));
        b[7] = quantize(
            precision, std::abs(std::abs(std::abs(c.imag()) - amp(8)) - amp(4)) - amp(2));
    }

    complex_type map(code_type* b)
    {
        return AMP * complex_type(
                         b[0] * (b[2] * (b[4] * (b[6] + value_type(2)) + value_type(4)) +
                                 value_type(8)),
...
struct QuadratureAmplitudeModulation<1024, TYPE, CODE> : public Modulation<TYPE, CODE> {
    ...
    void hard(code_type* b, complex_type c)
    {
        ...
        b[4] = std::abs(std::abs(c.real()) - amp(16)) < amp(8) ? code_type(-1)
                                                               : code_type(1);
        b[5] = std::abs(std::abs(c.imag()) - amp(16)) < amp(8) ? code_type(-1)
                                                               : code_type(1);
        ...
        b[7] = std::abs(std::abs(std::abs(c.imag()) - amp(16)) - amp(8)) < amp(4)
                   ? code_type(-1)
                   : code_type(1);
        b[8] = std::abs(std::abs(std::abs(std::abs(c.real()) - amp(16)) - amp(8)) -
                        amp(4)) < amp(2)
                   ? code_type(-1)
                   : code_type(1);
        b[9] = std::abs(std::abs(std::abs(std::abs(c.imag()) - amp(16)) - amp(8)) -
                        amp(4)) < amp(2)
                   ? code_type(-1)
                   : code_type(1);
    }
    ...
    complex_type map(code_type* b)
    {
        return AMP * complex_type(b[0] * (b[2] * (b[4] * (b[6] * (b[8] + value_type(2)) +
                                                          value_type(4)) +
                                                  value_type(8)) +
                                          value_type(16)),
                                  b[1] * (b[3] * (b[5] * (b[7] * (b[9] + value_type(2)) +
                                                          value_type(4)) +
                                                  value_type(8)) +
                                          value_type(16)));
    }
