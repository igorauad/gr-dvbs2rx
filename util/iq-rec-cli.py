...
def run_and_time(meta_data, impl='dvbs2-rx', extra_args=None):
    ...
    tic = time.time()
    runner(meta_data, out_file, extra_args)
    toc = time.time()
...
def rx(ctx, rec_name, impl, args):
    ...
    try:
        res = run_and_time(meta_data, impl=impl, extra_args=args)
...
def bench(ctx, rec_name):
    ...
    meta_data = repo_meta_data[rec_name]
    try:
        res_dvbs2rx = run_and_time(meta_data, impl='dvbs2-rx')
        res_leandvb = run_and_time(meta_data, impl='leandvb')
    except KeyboardInterrupt:
        return
    print(
