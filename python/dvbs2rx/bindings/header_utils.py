...
class PybindHeaderParser:
    ...
    def get_flags(self):
        return f'{self.get_flag_automatic()};{self.get_flag_pygccxml()};' + \
            f'{self.get_header_filename()};{self.get_header_file_hash()};'
...
def argParse():
    ...
    parser.add_argument(
        ...
        choices=[
            "flag_auto", "flag_pygccxml", "header_filename",
            "header_file_hash", "all"
        ])
    parser.add_argument(
        "pathname",
        help="Pathname of pybind c++ file to read, e.g. blockname_python.cc")
